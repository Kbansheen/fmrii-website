
<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>ManageEngine ServiceDesk Plus </title>



    <!-- BOOTSTRAP LINKS   -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js" integrity="sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN" crossorigin="anonymous"></script>

    <link rel="stylesheet" href="css/styles.css">

    <link rel="icon" href="favicon.ico">

    <!-- FONT AWESOME -->
    <script src="https://kit.fontawesome.com/3773351d80.js" crossorigin="anonymous"></script>

    <!-- Google fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;900&family=Ubuntu&display=swap" rel="stylesheet">

  </head>
  <body>

<div class="popup" id="x-popup">
  <div class="popup-content">
    <a target="_blank" class="no-style" href="https://www.googleadservices.com/pagead/aclk?sa=L&ai=DChcSEwjus9OgsNWBAxWMF3IKHahAAJUYABAAGgJzZg&ase=2&gclid=CjwKCAjwseSoBhBXEiwA9iZtxvxKjjv2vV5AS2hZOJGmncbZDg7np1KVnRCa0VrpJj_Te5F7_ojB5xoCBCoQAvD_BwE&ohost=www.google.com&cid=CAESVeD2N3c64OVN2QaYVx_OyduPc84L7uhbZkjOtoCITvHlscmnUGkFUg2htPFCuioIivBS6_Ms1eavg0jqtG1pkF6riGAvU0HYPAZk-UzugyYDiDNmKBw&sig=AOD64_2skotKFH4RfM0-GocikZ9e3BTP7g&q&nis=4&adurl&ved=2ahUKEwip98WgsNWBAxWuSWwGHQBBA2IQ0Qx6BAgPEAE"><h3>Zoho</h3></a>
    <p class="zoho">Zoho Corporation is a multinational technology company that offers a suite of
       cloud-based software applications for businesses. It was founded in 1996 by Sridhar Vembu and
        Tony Thomas, and is headquartered in Chennai, India. Zoho provides a wide range of software
        solutions designed to streamline various aspects of business operations, including customer
       relationship management (CRM), project management, accounting, human resources, marketing automation, and more.</p>
       <img class="z-img" src="data:image/png;base64," alt="">
    <a class="close" href="#">x</a>

  </div>
</div>

    <nav class="navbar navbar-expand-lg navbar-dark up-nav">
    <div class="container-fluid">

      <a class="navbar-brand bran" href="https://www.google.com/search?q=fortis&rlz=1C1NHXL_enIN1018IN1018&oq=fortis+&aqs=chrome..69i57j69i59j35i39i650j46i131i199i433i465i512j46i131i175i199i433i512j69i60l3.3676j0j7&sourceid=chrome&ie=UTF-8">
<img class="fortis" src="data:image/png;base64," alt="Logo" width="30" height="24" class="d-inline-block align-text-top">
<img class="ibm" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSh4rUVWDMjjaWBg4Z-GPl7e2d3gXJ6QcpcKQ&usqp=CAU" alt="Logo" width="30" height="24" class="d-inline-block align-text-top">
      </a>

      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarTogglerDemo01" aria-controls="#navbarTogglerDemo01" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item home">
            <a class="nav-link ho " aria-current="page" href="#">Home</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="dash.php">Dashboard</a>
          </li>

          <li class="nav-item">
            <a class="nav-link" href="display.php">Requests</a>
          </li>

          <li class="nav-item">
            <a class="nav-link " href="#">Problems</a>
          </li>

          <li class="nav-item">
            <a class="nav-link " href="#">Changes</a>
          </li>

          <li class="nav-item">
            <a class="nav-link " href="#">Projects</a>
          </li>

          <li class="nav-item">
            <a class="nav-link " href="#">Solutions</a>
          </li>

          <li class="nav-item">
            <a class="nav-link " href="#">Assets</a>
          </li>

          <li class="nav-item">
            <a class="nav-link ta " href="#">Reports</a>
          </li>

          <li class="nav-item glass">
            <form class="d-flex input-group w-auto">
            <input type="search"  class="form-control rounded search-bar" placeholder="Search" aria-label="Search" aria-describedby="search-addon"/>
            <a class="nav-link search " href="#"><i class="fa-solid fa-magnifying-glass"></i></a>
            <span>Search</span>
          </form>
          </li>

          <li class="nav-item">
            <a class="nav-link ta " href="form.php"><i class="fa-solid fa-hand"></i></a>
            <span>Raise request</span>
          </li>

          <li class="nav-item">
            <a class="nav-link ta" href="#"><i class="fa-solid fa-bolt"></i></a>
            <span>Quick actions</span>
          </li>

          <li class="nav-item">
            <a class="nav-link ta" href="#"><i class="fa-solid fa-clock-rotate-left"></i></a>
            <span>Recent items</span>
          </li>

          <li class="nav-item">
            <a class="nav-link ta" href="#"><i class="fa-regular fa-bell"></i></a>
            <span>Notifications</span>
          </li>

          <li class="nav-item">
            <a class="nav-link ta" href="#"><i class="fa-regular fa-circle-question"></i></a>
            <span>Help</span>
          </li>

          <li class="nav-item">
            <a class="nav-link ta" href="#"><i class="fa-solid fa-user"></i></a>
            <span>User profile</span>
          </li>

        </ul>

      </div>
    </div>
  </nav>

<div class="container-fluid title-container">
  <nav class="navbar  down-nav">
  <form class="container-fluid justify-content-start">
    <button class="btn btn-sm btn-outline-secondary" type="button">AD Self Service</button>
  <a href="#x-popup"><button class="btn btn-sm btn-outline-secondary second-button" type="button">Zoho Creator App</button></a>
    <button class="btn btn-sm btn-outline-secondary third-button" type="button"><i class="fa-solid fa-clipboard-list clipboard"></i></button>



  </form>
</nav>
</div>



<div class="middle-container">
  <div class="view">

    <nav class="navbar navbar-expand-lg bg-white view-nav">
    <div class="container-fluid">
      <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">


          <li class="nav-item ">
            <a class="nav-link my-view " aria-current="page" href="#"><i class="fa-solid fa-chart-pie pie"></i>My View</a>
          </li>

          <li class="nav-item sch">
            <a class="nav-link scheduler" href="#"><i class="fa-regular fa-calendar calendar"></i>Scheduler</a>
          </li>

        </ul>

      </div>
    </div>
  </nav>
</div>


<div class="row">
    <div class=" summ-div">
<div class=" summary">
<i class="fa-solid fa-book-open book"></i>
<h6 class="msumm">My Summary</h6>
</div>

<div class=" summ">
  <a href="#" class="table1">
<i class="fa-brands fa-stack-overflow boo"></i>
<p class="ms">Requests Overdue</p>
</a>
</div>
<div class=" pending">
  <a href="#" class="table1">
<i class="fa-solid fa-stopwatch boo"></i>
<p class="msu">Requests Due Today</p>
</a>
</div>
<div class="pending ">
  <a href="#" class="table1">
<i class="fa-solid fa-circle-xmark boo"></i>
<p class="msu">Pending Requests</p>
</a>
</div>
<div class="pending">
  <a href="#" class="table1">
<i class="fa-solid fa-thumbs-up boo"></i>
<p class="msu">Approved Changes</p>
</a>
</div>
<div class="pending">
  <a href="#" class="table1">
<i class="fa-solid fa-unlock boo"></i>
<p class="msu">Open Problems</p>
</a>
</div>
<div class="pending">
  <a href="#" class="table1">
<i class="fa-solid fa-bug boo"></i>
<p class="msu">Unassigned Problems</p>
</a>
</div>

  </div>

    <div class="task">
      <div class=" tasks">
<i class="fa-solid fa-clipboard clip"></i>
<h6 class="mtas">My Tasks</h6>
      </div>
      <img class="cb" src="https://media.istockphoto.com/id/1142349596/video/animation-of-a-clipboard-with-document-paper-listing-and-adding-tick-check-marks-flying-in.jpg?s=480x480&k=20&c=pwQM-H-EWiDkRBq0TLTgJr0ezw1frBozjAe7VRSFCYk=" alt="">
<p class="no">There are no tasks in this view</p>
      <button type="button" class="btn btn-outline-dark new"><i class="fa-solid fa-circle-plus plus"></i>New task</button>
</div>

    <div class="approvals">
      <div class=" summary">
<i class="fa-solid fa-person-circle-check person"></i>
<h6 class="msumm">My Approvals</h6>
      </div>
      <img class="tick"src="data:image/png;base64," alt="">
<p class="noo">No pending approvals</p>
    </div>

  </div>



<div class="row">
  <div class="app">
    <div class=" summary">
<i class="fa-solid fa-clock per"></i>
<h6 class="msumm">My Reminders</h6>
    </div>
    <img class="tick" src="https://media.istockphoto.com/id/1220402021/video/stopwatch-flat-animation-icon-design-moving-arrows-on-white-background.jpg?b=1&s=640x640&k=20&c=rQWcp9mhL1T1-8zM8gf3Dd2hgi9UdKgZi-csL2x25cw=" alt="">
<p class="noo">No reminder available</p>
<button type="button" class="btn btn-outline-dark old"><i class="fa-solid fa-circle-plus plus"></i>Add New</button>
  </div>

  <div id="testimonial-carousel" class="carousel slide" data-ride="false" >
        <div class="carousel-inner">

          <div class="carousel-item active">
                <h2 class="head">Services we offer</h2>
                <i class="fa-solid fa-check ci"></i>
                <h3 class="testimonial-text">Incident Management</h3>
                <p class="cp"> a centralized system for recording, following up on, and resolving IT incidents​</p>

          </div>

         <div class="carousel-item">
                <h2 class="head">Services we offer</h2>
              <i class="fa-solid fa-screwdriver-wrench ci"></i>
                <h3 class="testimonial-text">Service Request Management</h3>
                <p class="cp">a self-service interface for customers to submit and track service requests​​</p>

        </div>

        <div class="carousel-item">
               <h2 class="head">Services we offer</h2>
               <i class="fa-brands fa-stack-overflow ci"></i>
               <h3 class="testimonial-text">Change Management</h3>
               <p class="cp">easier to implement IT infrastructure changes, resulting in improved control over the change-processing workflow​​</p>

       </div>

       <div class="carousel-item">
              <h2 class="head">Services we offer</h2>
            <i class="fa-solid fa-database ci"></i>
              <h3 class="testimonial-text">Knowledge Base</h3>
              <p class="cp">The built-in knowledge base makes it possible to save fixes and troubleshooting advice, giving end users easy access to information ​</p>

      </div>

      <div class="carousel-item">
             <h2 class="head">Services we offer</h2>
            <i class="fa-solid fa-chart-line ci"></i>
             <h3 class="testimonial-text">Reports and Analytics</h3>
             <p class="cp"> helping users make well-informed decisions through a variety of pre-built reports and analytics​​</p>

     </div>

     <div class="carousel-item">
            <h2 class="head">Services we offer</h2>
            <i class="fa-solid fa-circle-exclamation ci"></i>
            <h3 class="testimonial-text">Problem Management</h3>
            <p class="cp"> locating, analyzing, and resolving reoccurring issues</p>

    </div>


   </div>
   <button class="carousel-control-prev " type="button" data-bs-target="#testimonial-carousel" data-bs-slide="prev">
     <span class="carousel-control-prev-icon " aria-hidden="true"></span>
     <span class="visually-hidden ">Previous</span>
   </button>

   <button class="carousel-control-next" type="button" data-bs-target="#testimonial-carousel" data-bs-slide="next">
     <span class="carousel-control-next-icon" aria-hidden="true"></span>
     <span class="visually-hidden">Next</span>
   </button>
 </div>


</div>

</div>
      </body>
    </html>
